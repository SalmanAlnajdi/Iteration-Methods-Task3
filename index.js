...
******************************************************************/
let greaterTemp = temperatures.filter( (temp)=> temp >= 25)

console.log(greaterTemp);

let lessTemp = temperatures.filter( (temp)=> temp < 20)
...
let fTemp = temperatures.map( (temp) => temp * 9/5 + 32)

console.log(fTemp);

let labelsTemp = temperatures.map( (temp)=>{
    if(temp >= 25){
        return "Warm"  
    }else if( 20 < temp < 24 ){
        return "Mild" 
...
let highestTemp= temperatures.reduce((highTemp,value)=> Math.max(highTemp,value))

 console.log(highestTemp);

 //let lowestTemp = Math.min(...temperatures)
 
 let lowestTemp = temperatures.reduce((lowTemp, value)=>Math.min(lowTemp,value))
